...
class FinancehubReconciliationController(http.Controller):
    ...
    def _get_suspense_lines(self, bank_line):
        ...
        In Odoo 17/18, account.bank.statement.line no longer has a .reconcile() method.
        Reconciliation is done via account.move.line.reconcile() on the suspense lines.
        """
    ...
    def _do_writeoff(self, bank_line, payload):
        """Create a write-off counterpart entry and reconcile via suspense account."""
        env = request.env
